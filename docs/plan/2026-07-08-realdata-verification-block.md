...
- [ ] **PR#4 step-reviewのPLAUSIBLE指摘**: `extract_dates()`の起算型regex（`日?\s*から起算して`）は
      「から」と「起算して」の間のスペースを許容していない（期間型は「から」前後・「まで」前後とも許容済み）。
      実データに起算型でスペース入り表記（例:「2025-05-13 から 起算して4週間」）の案件があれば
      同様に`\s*`を追加する要修正
