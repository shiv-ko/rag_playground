...
- [x] Q28型の対策（2026-07-06完了）: 根本原因は`.py`ファイルがTextParser未対応で常に
      「[未対応形式]」スタブだったこと（valid Q4/Q28が0/2で全滅していた実測と整合）。
      `.py`をSUPPORTEDに追加し、合わせて`chunk_documents`が行の途中でチャンクを切らないよう
      行境界（空行優先）へのスナップを実装。詳細は`docs/daily作業ログ/20260706_020135.md`。
      実データでのvalid再実行による効果測定は未実施（本セッションは`data/raw`無し環境のため）。
