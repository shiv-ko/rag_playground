...
- [x] 実装優先順位の確定（2026-07-18、期待改善問数×実装コスト）:
  ...
  4. [x] 第2波4件すべて実装・採用確定（2026-07-19、作業ログ`20260719_175544.md`）: fingerprint恒久策`ea4098f` / xlsxシートスコープ`7badf9a`（Q80誤答混入解消） / 暗号化検知内容ベース化`a388400`（KAEDEスケジュール5,803文字を索引化、根本原因は復号の本番経路未配線） / 回帰予測answerer`a6ce5cb`（Q63: -30.78416、Q83: 0.38317、丸めはDecimal ROUND_HALF_UP）。採用ゲート: recall 25/29不変・wave2 valid N=3 local 0.85/0.85/0.83・Incorrect全run 0・gate/path変化ゼロ。
...
- [x] **valid N=3＋flip分析（最終採用ゲート）**: ユーザー承認後2026-07-19実施（`wave1_valid_r1〜r3`）。local mean 0.8333/0.8167/0.8333、**Incorrect全run 0でゲート通過**。回答テキスト差分はr1: 2問・r2: 3問（#8/#9/#12、既知の生成揺れ）・r3: 0問で、gate反転・path変化ゼロ＝第1波はvalidの回答経路に無影響。ベースライン比の見かけのmean改善（0.65→0.83）はjudgeプロンプト改善（`3413b52`）による同一回答への判定変化。**第1波3コミット採用確定**。詳細は作業ログ`20260719_020500.md`。
...
- [x] test N=3提出物生成（2026-07-19、`submission_20260719_wave1-3run.zip`）: HEAD固定worktree（`7edcfb0`）から`make_predictions.py --runs 3`。**all_missing 57→53、最終回答数36→40**。新規回答7問のうちQ16「4,620,000、0.589」（診断の正解候補一致）/Q99「2.49倍」/Q96/Q11は第1波の狙いどおり、Q8/Q41/Q97消失は既知のバッチ間揺れ。CSV検証済み（100 ID網羅・エラーなし・最長104文字）。**SIGNATE手動提出待ち**。
  - 実行時の注意2件: ①worktreeに`.env`/`.cache`のsymlink必須（初回は全問APIキーエラーで空振り、メモリ記録済み） ②run中に散発的なAPI接続エラー「Cannot send a request, as the client has been closed」が2run×計8問で発生（リトライで全問回復、最終出力に影響なし）。後者は本番3時間制限下での信頼性リスクとして下記に追加。
- [ ] **Anthropicクライアントのライフサイクル調査**: 並行スレッド利用中のclient closeが疑われる。再現条件の特定と、リトライで吸収しきれない場合の影響評価（第2波と並行の調査タスク候補）。
